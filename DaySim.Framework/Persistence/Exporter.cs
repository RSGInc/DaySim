...
namespace DaySim.Framework.Persistence {
  public abstract class Exporter<TModel> : IExporter<TModel> where TModel : IModel, new() {
    ...
    private void WriteHeader() {
      ...
      System.Collections.Generic.List<string> fields = properties.Select(property => property.GetCustomAttributes(typeof(ColumnNameAttribute), true).Cast<ColumnNameAttribute>().SingleOrDefault()).Where(attribute => attribute != null).Select(attribute => attribute.ColumnName).ToList();
      int i = 0;
