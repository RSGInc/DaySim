...
namespace DaySim.Framework.Sampling {
  ...
  public sealed class SegmentZone {
    ...
    public IEnumerable<SizeSegmentItem> RankedSizes => _rankedSizeIndices.Select(index => new SizeSegmentItem(index, _parcelSizes[index].Id, _parcelSizes[index].Value));

    public IEnumerable<WeightSegmentItem> RankedWeights => _rankedWeightIndices.Select(index => new WeightSegmentItem(index, _zoneWeights[index]));
    ...
    public static void Save(SegmentZone segmentZone, BinaryWriter writer) {
      ...
      foreach (ParcelSize item in segmentZone._parcelSizes) {
        writer.Write(item.Id);
        writer.Write(item.Value);
      }

      foreach (int index in segmentZone._rankedSizeIndices) {
        writer.Write(index);
      }

      foreach (double weight in segmentZone._zoneWeights) {
        writer.Write(weight);
      }

      foreach (int index in segmentZone._rankedWeightIndices) {
        writer.Write(index);
      }
    }
    ...
    public static SegmentZone Load(BinaryReader reader) {
      ...
      SegmentZone segmentZone = new SegmentZone(parcelCount, zoneCount) {
        Id = reader.ReadInt32(),
