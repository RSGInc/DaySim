          ...
          if (!(Global.Configuration.IsInEstimationMode)) {
            ...
            if (halfTourTrips[i] == halfTourTrips[i - 1]) {
              ...
              tour[i].DestinationArrivalTime = Math.Max(1,tour[i - 1].DestinationArrivalTime
                   - (int)Math.Round(ImpedanceRoster.GetValue(impedanceVariable, mode, pathType, Math.Max(Global.Configuration.VotLowMedium + 1, 10.0), Math.Max(tour[i - 1].DestinationArrivalTime - 1,1), tour[i].DestinationParcel, tour[i - 1].DestinationParcel, circuityDistance).Variable)
                   - 3);
              tour[i].DestinationDepartureTime = Math.Min(1440, tour[i - 1].DestinationDepartureTime
                   + (int)Math.Round(ImpedanceRoster.GetValue(impedanceVariable, mode, pathType, Math.Max(Global.Configuration.VotLowMedium + 1, 10.0), Math.Min(tour[i - 1].DestinationDepartureTime + 1,1440), tour[i - 1].DestinationParcel, tour[i].DestinationParcel, circuityDistance).Variable)
                   + 3);
            }
          }
