...
namespace DaySim.ChoiceModels.Actum.Models {
  public class TourDestinationModel : ChoiceModel {
    ...
    private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, TourWrapper tour, HouseholdDayWrapper householdDay, int sampleSize, IParcelWrapper choice = null) {
      ...
      System.Collections.Generic.Dictionary<DestinationSampler.TourSampleItem, int> sampleItems = destinationSampler.SampleAndReturnTourDestinations(tourDestinationUtilities);

      int index = 0;
      foreach (System.Collections.Generic.KeyValuePair<DestinationSampler.TourSampleItem, int> sampleItem in sampleItems) {
        ...
        if (tour.IsHomeBasedTour) {
          if (tour.DestinationPurpose == Global.Settings.Purposes.Work) {
            ...
            ChoiceProbabilityCalculator.Alternative nestedAlternative = Global.ChoiceModelSession.Get<WorkTourModeTimeModel>().RunNested(tour, destinationParcel, tour.Household.VehiclesAvailable, tour.Person.GetTransitFareDiscountFraction());
          ...
          //}
            ...
            ChoiceProbabilityCalculator.Alternative nestedAlternative = Global.ChoiceModelSession.Get<OtherHomeBasedTourModeTimeModel>().RunNested(tour, destinationParcel, tour.Household.VehiclesAvailable, tour.Person.GetTransitFareDiscountFraction());
            tourLogsum = nestedAlternative == null ? 0 : nestedAlternative.ComputeLogsum();
          }
        } else {
          // JLB201406
          //var nestedAlternative = Global.ChoiceModelSession.Get<WorkBasedSubtourModeModel>().RunNested(tour, destinationParcel);
          ChoiceProbabilityCalculator.Alternative nestedAlternative = Global.ChoiceModelSession.Get<WorkBasedSubtourModeTimeModel>().RunNested(tour, destinationParcel, tour.Household.VehiclesAvailable, tour.Person.GetTransitFareDiscountFraction());
          tourLogsum = nestedAlternative == null ? 0 : nestedAlternative.ComputeLogsum();
        }
