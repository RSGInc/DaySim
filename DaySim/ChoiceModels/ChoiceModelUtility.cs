...
namespace DaySim.ChoiceModels {
  public static class ChoiceModelUtility {
    ...
    private static readonly double[,,,] _timeOfDayFractionActum = new[, , ,] {
			// NONE_OR_HOME
    ...
    private static readonly int[,] _arrivalTimeMinuteActum = new[,] { { 300, 300, 300, 300 }, { 0, 540, 540, 540 }, { 0, 0, 840, 840 }, { 120, 120, 120, 1020 } };

    private static readonly int[,] _departureTimeMinuteActum = new[,] { { 360, 600, 840, 1020 }, { 0, 600, 840, 1020 }, { 0, 0, 900, 1020 }, { 360, 600, 840, 1140 } };
    ...
    public static void DrawRandomTourTimePeriodsActum(ITourWrapper tour, int tourCategory) {
      if (tour == null) {
        throw new ArgumentNullException("tour");
      }
      ...
      while (random >= 0 && arrivalPeriodIndex < endTimePeriod || departurePeriodIndex < endTimePeriod) {
        ...
        random = random - _timeOfDayFractionActum[tour.DestinationPurpose, tourCategory, arrivalPeriodIndex, departurePeriodIndex];
      }

      tour.DestinationArrivalTime = _arrivalTimeMinuteActum[arrivalPeriodIndex, departurePeriodIndex];
      tour.DestinationDepartureTime = _departureTimeMinuteActum[arrivalPeriodIndex, departurePeriodIndex];
      if (checkAgainstDaySim) {
        int startAndEndPeriod = 0;
        double cumulativePercent = 0;
        while (randomDaySimCopy > cumulativePercent & startAndEndPeriod < _arrivalTimeMinute.Length) {
          cumulativePercent = cumulativePercent + _timeOfDayFraction[tour.DestinationPurpose, tourCategory, startAndEndPeriod];
          if (randomDaySimCopy < cumulativePercent) { break; }
          startAndEndPeriod = startAndEndPeriod + 1;
        }
        if (randomDaySimCopy > cumulativePercent) {
          startAndEndPeriod = startAndEndPeriod - 1; //in case fractions do not quite sum to 1 and random is greater
        }

        int daySimDestinationArrivalTime = _arrivalTimeMinute[startAndEndPeriod];
        int daySimDestinationDepartureTime = _departureTimeMinute[startAndEndPeriod];
        Global.PrintFile.WriteLine("DrawRandomTourTimePeriodsActum ACTUM arrivalPeriodIndex={0}, departurePeriodIndex={1}, _arrivalTimeMinuteActum[arrivalPeriodIndex, departurePeriodIndex]={2}, _departureTimeMinuteActum[arrivalPeriodIndex, departurePeriodIndex]={3}", arrivalPeriodIndex, departurePeriodIndex, _arrivalTimeMinuteActum[arrivalPeriodIndex, departurePeriodIndex], _departureTimeMinuteActum[arrivalPeriodIndex, departurePeriodIndex]);
        Global.PrintFile.WriteLine("DrawRandomTourTimePeriodsActum DAYSIM startAndEndPeriod={0}, _arrivalTimeMinute[startAndEndPeriod]={1}, _departureTimeMinute[startAndEndPeriod]={2}", startAndEndPeriod, _arrivalTimeMinute[startAndEndPeriod], _departureTimeMinute[startAndEndPeriod]);
      }
    }
