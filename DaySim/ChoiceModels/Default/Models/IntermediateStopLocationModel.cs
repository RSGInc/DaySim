	...
	public class IntermediateStopLocationModel : ChoiceModel {
		...
		private sealed class IntermediateStopLocationUtilities : ISamplingUtilities {
			...
			public void SetUtilities(ISampleItem sampleItem, int sampleFrequency) {
				...
				var household = _trip.Household;
				var person = _trip.Person;
				var tour = _trip.Tour;
				...
				// stop purpose
				var workDestinationPurposeFlag = _trip.IsWorkDestinationPurpose().ToFlag();
				var schgFlag = (_trip.IsSchoolDestinationPurpose() && (person.IsChildAge5Through15 || person.IsChildUnder5)).ToFlag();
				var schhFlag = (_trip.IsSchoolDestinationPurpose() && person.IsDrivingAgeStudent).ToFlag();
				var schuFlag = (_trip.IsSchoolDestinationPurpose() && (!person.IsChildAge5Through15 && !person.IsChildUnder5) && !person.IsDrivingAgeStudent).ToFlag();
				var workOrSchoolDestinationPurposeFlag = _trip.IsWorkOrSchoolDestinationPurpose().ToFlag();
				var eskdFlag = (_trip.IsEscortDestinationPurpose() && (household.HouseholdTotals.DrivingAgeStudents + household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 > 0)).ToFlag();
				var esnkFlag = (_trip.IsEscortDestinationPurpose() && (household.HouseholdTotals.DrivingAgeStudents + household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 == 0)).ToFlag();
				//#if TRACE
				//				var personalBusinessDestinationPurposeFlag = _trip.IsPersonalBusinessDestinationPurpose.ToFlag();
				//#endif
				var shoppingDestinationPurposeFlag = _trip.IsShoppingDestinationPurpose().ToFlag();
				var mealDestinationPurposeFlag = _trip.IsMealDestinationPurpose().ToFlag();
				var socialDestinationPurposeFlag = _trip.IsSocialDestinationPurpose().ToFlag();
				var recreationDestinationPurposeFlag = _trip.IsRecreationDestinationPurpose().ToFlag();
				var srec = (socialDestinationPurposeFlag == 1 || recreationDestinationPurposeFlag == 1).ToFlag();
				var personalReasonsDestinationPurposeFlag = _trip.IsPersonalReasonsDestinationPurpose().ToFlag();
				var personalDestinationPurposeFlag = (personalReasonsDestinationPurposeFlag == 1 || recreationDestinationPurposeFlag == 1).ToFlag();
				var personalOrMedicalDestinationPurposeFlag = _trip.IsPersonalBusinessOrMedicalDestinationPurpose().ToFlag();

				// tour and trip characteristics
				var shshFlag = (_trip.IsShoppingDestinationPurpose() && tour.IsShoppingPurpose()).ToFlag(); // shoppingDestinationPurposeFlag trip on shoppingDestinationPurposeFlag tour
				var bmanFlag = (_trip.IsHalfTourFromOrigin && (tour.IsWorkPurpose() || tour.IsSchoolPurpose())).ToFlag();
				var parkingTime = _averageStopDuration[_trip.DestinationPurpose] / 6000D; // parking time (hrs)
				var notFirstFlag = (_trip.Sequence != 1).ToFlag();

				var empo = destinationParcel.EmploymentGovernment + destinationParcel.EmploymentOffice + destinationParcel.EmploymentEducation;
				var empi = destinationParcel.EmploymentIndustrial + destinationParcel.EmploymentAgricultureConstruction;
				var enrs = destinationParcel.StudentsK8 + destinationParcel.StudentsHighSchool;

				var empsq = Math.Log(destinationParcel.EmploymentServiceBuffer1 + 1);
				var empmq = Math.Log(destinationParcel.EmploymentMedicalBuffer1 + 1);
				var emprq = Math.Log(destinationParcel.EmploymentRetailBuffer1 + 1);
				var empfq = Math.Log(destinationParcel.EmploymentFoodBuffer1 + 1);
				...
				var empiq = Math.Log(destinationParcel.EmploymentIndustrialBuffer1 + destinationParcel.EmploymentAgricultureConstructionBuffer1 + 1);
				var emptq = Math.Log(destinationParcel.EmploymentTotalBuffer1 + 1);
				var hhldxq = Math.Log(destinationParcel.HouseholdsBuffer1 + 1);
				var enrsq = Math.Log(destinationParcel.StudentsK8Buffer1 + destinationParcel.StudentsHighSchoolBuffer1 + 1);
				var enruq = Math.Log(destinationParcel.StudentsUniversityBuffer1 + 1);
				//				var parkq = Math.Log(destinationParcel.ParkingOffStreetPaidHourlySpacesBuffer1 + 1);
				var openqFlag = (destinationParcel.OpenSpaceType2Buffer1 > 0).ToFlag();
				var n134Q = destinationParcel.NodesSingleLinkBuffer1 + destinationParcel.NodesThreeLinksBuffer1 + destinationParcel.NodesFourLinksBuffer1;
				...
				var gtim = (gtim1 + gtim2) / 100.0; // approximate incremental gtime, rescale to units of (100 minutes)  //rescale to units of (100 minutes
				...
				var twin = ttim / (Math.Max(1.0, _availableWindow) / 100D); // unit-free ratio of ttim and window, both measured in 100 minute units
				...
				var disc = Math.Min(0.125, diss * dist);
				...
				var fkid = (person.Gender == Global.Settings.PersonGenders.Female && person.PersonType < Global.Settings.PersonTypes.DrivingAgeStudent && (household.HouseholdTotals.ChildrenAge5Through15 + household.HouseholdTotals.ChildrenUnder5 + household.HouseholdTotals.DrivingAgeStudents > 0)).ToFlag();
				//#if TRACE
				...
				if (Global.Configuration.ShouldUseODShadowPricing) {
					var stopOriginShadowPriceConfigurationParameter = _trip.OriginParcel.District == destinationParcel.District? Global.Configuration.IntermediateStopLocationOOShadowPriceCoefficient : Global.Configuration.IntermediateStopLocationODShadowPriceCoefficient;
					var tourOriginShadowPriceConfigurationParameter = _trip.Tour.OriginParcel.District == destinationParcel.District? Global.Configuration.IntermediateStopLocationOOShadowPriceCoefficient : Global.Configuration.IntermediateStopLocationODShadowPriceCoefficient;
