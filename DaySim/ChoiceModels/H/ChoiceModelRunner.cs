  ...
  public sealed class ChoiceModelRunner : IChoiceModelRunner {
    ...
    private void RunHouseholdModels() {
      ...
      RunHouseholdModelSuite(_household);
        ...
        throw new DaySim.Framework.Exceptions.HouseholdModelException(string.Format("Error running household models for {0}.", _household), e);
      }
#endif
    }
    ...
    private void RunPersonModels() {
      ...
      foreach (IPersonWrapper person in _household.Persons) {
        ...
        RunPersonModelSuite(person);
          ...
          throw new DaySim.Framework.Exceptions.PersonModelException(string.Format("Error running person models for {0}.", person), e);
        }
#endif
      }
    }
    ...
    private void RunHouseholdDayModels() {
      ...
      foreach (HouseholdDayWrapper householdDay in _household.HouseholdDays) {
        ...
        while (!householdDay.IsValid && (!Global.Configuration.IsInEstimationMode || !simulatedAnInvalidHouseholdDay)) { //don't retry household in estimation mode
          ...
          throw new DaySim.Framework.Exceptions.HouseholdDayModelException(string.Format("Error running household day models for {0}.", _household), e);
        }
#endif
    ...
    private void RunSubtourModels(TourWrapper tour, PersonDayWrapper personDay, HouseholdDayWrapper householdDay) {
      ...
      foreach (ITourWrapper subtour in tour.Subtours) {
        ...
        if (!subtour.PersonDay.IsValid) {
          ...
          throw new DaySim.Framework.Exceptions.SubtourModelException(string.Format("Error running subtour models for {0}.", subtour), e);
        }
#endif
      }
    }
    ...
    private void RunHouseholdDayModelSuite(HouseholdDayWrapper householdDay) {
      ...
      if (Global.Configuration.ShouldRunHouseholdDayPatternTypeModel) {
        ...
        foreach (PersonDayWrapper personDay in orderedPersonDays) {
          i++;
          if (i > 5 || (i > 4 && Global.Configuration.IsInEstimationMode && Global.Configuration.EstimationModel == DaySim.ChoiceModels.H.Models.PersonDayPatternTypeModel.CHOICE_MODEL_NAME)) {
      ...
      foreach (IPersonDayWrapper personDay in householdDay.PersonDays) {
        ...
        if (!Global.Configuration.IsInEstimationMode) {
          ...
          throw new DaySim.Framework.Exceptions.PersonDayModelException(string.Format("Error running person-day models for {0}.", personDay), e);
        }
#endif
      }
      ...
      foreach (IPartialHalfTourWrapper partialJointHalfTour in householdDay.PartialHalfToursList) {
        ...
        RunPartialJointHalfTourModelSuite(householdDay, (PartialHalfTourWrapper)partialJointHalfTour);
      ...
      foreach (IFullHalfTourWrapper fullJointHalfTour in householdDay.FullHalfToursList) {
        ...
        RunFullJointHalfTourModelSuite(householdDay, (FullHalfTourWrapper)fullJointHalfTour);
        if (householdDay.IsValid == false) {
          return;
        }
      }
      ...
      foreach (IPersonDayWrapper personDay in householdDay.PersonDays) {
        foreach (ITourWrapper tour in personDay.Tours) {
          if (tour.DestinationPurpose == Global.Settings.Purposes.Work || tour.DestinationPurpose == Global.Settings.Purposes.School) {
            ...
            RunMandatoryTourModelSuite((TourWrapper)tour, (PersonDayWrapper)personDay, householdDay);
      ...
      foreach (IJointTourWrapper jointTour in householdDay.JointToursList) {
        ...
        RunJointTourModelSuite(householdDay, (JointTourWrapper)jointTour);
      ...
      foreach (IPersonDayWrapper personDay in householdDay.PersonDays) {
        ...
        foreach (ITourWrapper tour in personDay.Tours) {
          ...
          if (!(tour.DestinationPurpose == Global.Settings.Purposes.Work || tour.DestinationPurpose == Global.Settings.Purposes.School)) {
            ...
            RunNonMandatoryTourModelSuite((TourWrapper)tour, (PersonDayWrapper)personDay, householdDay);
    ...
    private void RunNonMandatoryTourModelSuite(TourWrapper tour, PersonDayWrapper personDay, HouseholdDayWrapper householdDay) {
      ...
      UpdateTimeWindowForTourDestinationTimes(tour);
        ...
        throw new DaySim.Framework.Exceptions.TourModelException(string.Format("Error running tour models for {0}.", tour), e);
      }
#endif
    ...
    private void SetTourDestination(ITourWrapper tour, IHouseholdDayWrapper householdDay, IParcelWrapper constrainedParcel = null) {
      ...
      if (tour.DestinationPurpose == Global.Settings.Purposes.Work) {
        ...
        if (Global.Configuration.IsInEstimationMode && Global.Configuration.EstimationModel == DaySim.ChoiceModels.H.Models.AggregateTourModeDestinationModel.CHOICE_MODEL_NAME) {
    ...
    private void SetSubtourDestination(ITourWrapper subtour, IHouseholdDayWrapper householdDay) {
      ...
      if (Global.Configuration.IsInEstimationMode && Global.Configuration.EstimationModel == DaySim.ChoiceModels.H.Models.AggregateTourModeDestinationModel.CHOICE_MODEL_NAME && subtour.DestinationPurpose != Global.Settings.Purposes.School) {
    ...
    private void RunPartialHalfTourTripModelSuite(ITourWrapper tour, IHalfTour halfTour, ITripWrapper trip, int stopPurpose, IParcelWrapper stopLocation, int mode, int departureTime) {
      ...
      ITripWrapper nextTrip = GeneratePartialHalfTourIntermediateStop(halfTour, trip, stopPurpose, stopLocation, mode, departureTime);
      ...
      for (int direction = firstDirection; direction <= lastDirection; direction++) {
        ...
        for (int i = 0; i < halfTour.Trips.Count; i++) {
          ...
          if (!trip.PersonDay.IsValid) {
            ...
            throw new DaySim.Framework.Exceptions.TripModelException(string.Format("Error running trip models for {0}.", trip), e);
          }
#endif
        }
      }
    ...
    private void RunTripModelSuite(IHouseholdDayWrapper householdDay, IPersonDayWrapper personDay, ITourWrapper tour, IHalfTour halfTour, ITripWrapper trip) {
      ...
      ITripWrapper nextTrip = GenerateIntermediateStop(halfTour, trip, householdDay);
    ...
    private void RunTripCloneSuite(ITourWrapper sourceTour, IHalfTour sourceHalfTour, ITripWrapper sourceTrip, ITourWrapper tour, IHalfTour halfTour, ITripWrapper trip) {
      ...
      ITripWrapper nextTrip = GenerateIntermediateStopClone(sourceHalfTour, sourceTrip, halfTour, trip);
