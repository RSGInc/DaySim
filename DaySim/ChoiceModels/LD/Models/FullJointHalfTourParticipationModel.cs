...
namespace Daysim.ChoiceModels.LD.Models {
	public class FullJointHalfTourParticipationModel : ChoiceModel {
		private const string CHOICE_MODEL_NAME = "LDFullJointHalfTourParticipationModel";
		...
		private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, HouseholdDayWrapper householdDay, int jHTSimulated, int genChoice, bool[,] jHTAvailable, bool[] fHTAvailable, int[][] altParticipants, int choice = Constants.DEFAULT_VALUE) {
			...
			foreach (PersonDayWrapper personDay in orderedPersonDays) {
				...
				if (count <= 5) {
					...
					pType8[count] = personDay.Person.IsChildUnder5.ToFlag(); // All LD TU persons are one of Type 1 to 8 
			...
			for (int alt = 0; alt < 32; alt++) {
				...
				//var nestedAlternative = LDWorkTourModeModel.RunNested(pPersonDay, pPersonDay.Household.ResidenceParcel, sameUsualLocation, destinationArrivalTime, destinationDepartureTime, pPersonDay.Household.VehiclesAvailable);
				//var nestedAlternative = (Global.ChoiceModelDictionary.Get("LDWorkTourModeModel") as LDWorkTourModeModel).RunNested(personDay, residenceParcel, person.UsualWorkParcel, destinationArrivalTime, destinationDepartureTime, household.VehiclesAvailable);
				//JLB 201406
