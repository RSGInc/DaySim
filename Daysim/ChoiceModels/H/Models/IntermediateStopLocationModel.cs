	...
	public class IntermediateStopLocationModel : ChoiceModel{
		...
		private sealed class IntermediateStopLocationUtilities : ISamplingUtilities {
			...
			public void SetUtilities(ISampleItem sampleItem, int sampleFrequency) {
				...
				var personalBusiness_Meal_Shop_Social_Or_RecreationStopPurposeFlag = (personalReasonsDestinationPurposeFlag == 1 || recreationStopPurposeFlag == 1).ToFlag();
				...
				var dist = Math.Max(adis1 + adis2 - adis0, Math.Min(adis1 * 0.1, adis2 * 0.1)); // incremental distance (with imposed lower limit), in distance units
