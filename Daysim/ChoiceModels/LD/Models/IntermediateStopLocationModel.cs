...
namespace Daysim.ChoiceModels.LD.Models {
	public class IntermediateStopLocationModel : ChoiceModel {
		private const string CHOICE_MODEL_NAME = "LDIntermediateStopLocationModel";
			...
			public void SetUtilities(ISampleItem sampleItem, int sampleFrequency) {
				...
				if (tour.Mode == Global.Settings.Modes.Walk) {
					...
					if (wdis1 < WTHRESH && wdis2 < WTHRESH && wdis1 > Constants.EPSILON && wdis2 > Constants.EPSILON) {
						...
						if (_trip.Direction == Global.Settings.TourDirections.OriginToDestination) {
							GetGenTime(_trip.Household.RandomUtility, Global.Settings.Modes.HovDriver, purpose, costCoef, timeCoef, transitDiscountFraction, 1, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
							GetGenTime(_trip.Household.RandomUtility, Global.Settings.Modes.HovDriver, purpose, costCoef, timeCoef, transitDiscountFraction, 2, tour.OriginParcel, destinationParcel, _trip.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
						}
						else {
							GetGenTime(_trip.Household.RandomUtility, Global.Settings.Modes.HovDriver, purpose, costCoef, timeCoef, transitDiscountFraction, 1, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis1, out d2Time1, out gd2Time1, _destinationDepartureTime, parkingTime);
							GetGenTime(_trip.Household.RandomUtility, Global.Settings.Modes.HovDriver, purpose, costCoef, timeCoef, transitDiscountFraction, 2, _trip.OriginParcel, destinationParcel, tour.OriginParcel, out d2Dis2, out d2Time2, out gd2Time2, _destinationDepartureTime, parkingTime);
						}
